...
{{--                            <strong>{{ \Carbon\Carbon::parse($status->updated_at)->toDayDateTimeString() }}</strong>--}}
                ...
                <div class="row">
                    <form action="{{ route('admin.update-status') }}" method="POST">
                        ...
                        <div class="row">
                            <div class="col-md-8">
                                <input type="hidden" class="form-control form-control-sm text-center" name="bookingid" value="{{ $booking->id }}">
                                ...
                                <select class="form-control text-secondary small" name="status_value" id="">
                                    <option value="1" {{ 1 === $status->current_status ? 'selected' : '' }}>No Progress</option>
                                    <option value="2" {{ 2 === $status->current_status ? 'selected' : '' }}>All Photos Uploaded</option>
                                    <option value="3" {{ 3 === $status->current_status ? 'selected' : '' }}>Wedding Photobook Delivered</option>
                                    <option value="4" {{ 4 === $status->current_status ? 'selected' : '' }}>Video Uploaded</option>
                                    <option value="5" {{ 5 === $status->current_status ? 'selected' : '' }}>Printing 16R Photos</option>
                                </select>
                            </div>
